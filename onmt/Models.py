...
class RNNEncoder(EncoderBase):
    ...
    def forward(self, src, lengths=None, encoder_state=None):
        ...
        s_len, batch, emb_dim = emb.size()
        is_sorted = lambda a : torch.ge(a[:-1], a[1:]).all()
        packed_emb = emb
        changedorder = False
        if lengths is not None:
            if not is_sorted(lengths):
                sorted_lengths, inds = torch.sort(lengths, descending = True)
                emb = emb.index_select(1, inds)
                changedorder = True
                packed_emb = pack(emb, sorted_lengths.data.tolist())
            else:
        ...
        if changedorder:
            _, rinds = torch.sort(inds)
            memory_bank = memory_bank.index_select(1, rinds)
            h_n, c_n = encoder_final
            encoder_final = (h_n.index_select(1, rinds), c_n.index_select(1, rinds))
...
class StdRNNDecoder(RNNDecoderBase):
    ...
    def _build_rnn(self, rnn_type, **kwargs):
        rnn  = rnn_factory(rnn_type, **kwargs)
        return rnn
